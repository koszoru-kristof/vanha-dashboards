...
| Coordinator | SMLIGHT **[SLZB-MR4U](https://smlight.tech/en-US/products/slzb-mrxu)** (dual radio, PoE) | settled |
| Thread border router | The MR4U's own Silabs radio, running OTBR in HA | settled |
| Temp/humidity | [IKEA TIMMERFLOTTE](https://www.ikea.com/fi/fi/p/timmerflotte-laempoetila-ilmankosteusanturi-aelylaite-30597606/) | leaning |
| Presence | [Aqara FP300](https://smarthomescene.com/reviews/aqara-fp300-presence-multi-sensor-teardown-and-review/) | leaning |
| Lights | IKEA KAJPLATS, Hue where quality matters | open |
| Switches/relays | [Shelly Gen4](https://www.shelly.com/products/shelly-1pm-gen4) in ceiling boxes | open, needs wiring survey |

---

## 1. Platform and protocol model

The [mental model](https://openelab.io/blogs/learn/matter-vs-zigbee-vs-thread-home-assistant) that matters, because it is easy to get wrong:

- **Zigbee** — self-contained mesh with its own coordinator. Mature, cheapest
  devices, best battery life, richest vendor features
  ([why Zigbee for Home Assistant](https://www.howtogeek.com/why-you-should-choose-zigbee-instead-of-thread-for-your-home-assistant-server/)).
- **Thread** — an IPv6 mesh. Just transport. Carries Matter.
- **Matter** — the application layer. Defines what a device *exposes*.
- **[Thread Border Router (TBR)](https://www.matteralpha.com/frequently-asked-questions/thread-border-routers-the-complete-list)** — an IPv6 router bridging the Thread mesh to
...
Worked example, the [Aqara FP300](https://www.derekseaman.com/2025/11/aqara-fp300-the-ultimate-presence-sensor-home-assistant-edition.html):
...
[Aqara state](https://forum.aqara.com/t/fp300-matter-over-thread-vs-zigbee-functionality/243947) they "strictly follow the standard when implementing Matter over
Thread" and deliberately do not implement manufacturer-specific clusters.

[Generally lost over Matter](https://www.xda-developers.com/matter-promise-is-a-mess-and-im-going-back-to-zigbee-only-devices/): deep energy monitoring, pixel-level LED control,
...
### Chosen: [SMLIGHT SLZB-MR4U](https://smlight.tech/en-US/products/slzb-mrxu)
...
| Price | **EUR 55.99** [Domadoo (FR)](https://www.domadoo.fr/en/403_smlight) / **EUR 66.00** [Nurkan takaa (Hyrylä, FI)](https://verkkokauppa.nurkantakaa.fi/tuote/smlight-slzb-mr4/) |
...
[SMLIGHT's own FAQ](https://smlight.tech/en-US/products/slzb-mrxu): *"Only the two radio SoCs change. The ESP32-S3 core, the
...
| | Zigbee radio (TI) | Thread radio (Silabs) | [Domadoo](https://www.domadoo.fr/en/403_smlight) |
|---|---|---|---|
...
Notes: [MR1U](https://smarthomescene.com/reviews/smlight-slzb-mr1-multi-radio-coordinator-setup-and-review/) is dominated by MR3U (older chips for EUR 1 less). MR3U is the safe
...
- **Never power via PoE and USB-C simultaneously.** SMLIGHT's marketing claims
  optoelectronic isolation makes it safe; the [retailer spec sheet](https://www.smarterhomeshop.eu/SLZB-MR1U-Zigbee-Thread-Ethernet-PoE-USB-LAN-WIFI) and [Howmation's
  review](https://howmation.com/en_US/product/smlight-slzb-mr5u-zigbee-thread-ethernet-poe-dual-efr32mg24-usb-passthrough-coordinator) both say it risks damaging the board. Use one source.
...
| Google Home Speaker (2026), EUR 119.99 | Is a real TBR, but: [black box](https://medium.com/engineering-iot/why-you-cant-use-your-own-openthread-border-router-with-google-apple-or-amazon-b413b6b0ab9e) with no OTBR UI/diagnostics/firmware control; HA must import Thread credentials via the Android companion app, which has a [known bug](https://github.com/home-assistant/core/issues/177007) importing a stale dataset instead of the active network; fallback path drags Google's cloud into local automations and causes multi-admin "popcorn" latency. EUR 120 vs EUR 56 for one we control. |
| [HA Connect ZBT-2](https://www.home-assistant.io/connect/zbt-2/) as a second radio | Fine in itself, but the MR4U makes it unnecessary. |
| [ZBT-2](https://www.home-assistant.io/connect/zbt-2/) attached to an [SLZB-06](https://smlight.tech/en-US/products/slzb-06u) via USB passthrough | The ZBT-2 uses an **[ESP32-S3 as its USB-serial bridge](https://support.nabucasa.com/hc/en-us/articles/31313138988701-About-Home-Assistant-Connect-ZBT-2-firmware-options)** (native USB CDC), not a CP210x/CH340. SMLIGHT's compatibility list does not cover it despite a marketing photo showing exactly this. Also unclear whether internal radio + passthrough run concurrently, and OTBR over a serial tunnel is timing-sensitive. |
...
|---|---|---|---|
| **[IKEA TIMMERFLOTTE](https://www.ikea.com/fi/fi/p/timmerflotte-laempoetila-ilmankosteusanturi-aelylaite-30597606/)** | Matter/Thread | **EUR 9.99** | Display, AAA. Needs a TBR. Best value by a wide margin |
...
| Device | Protocol | Price | Notes |
|---|---|---|---|
| **[Aqara FP300](https://smarthomescene.com/reviews/aqara-fp300-presence-multi-sensor-teardown-and-review/)** | **Thread or Zigbee, switchable** | EUR 40.90–49 (Proshop / Nurkan takaa) | mmWave + PIR hybrid, plus temp/humidity/lux. 3 yr battery Zigbee, 2 yr Thread. Battery-powered mmWave is the notable bit |
| Aqara FP1E | Zigbee | EUR 51.99 | Mains USB, single zone |
| Aqara FP2 | Wi-Fi | ~EUR 70+ | Multi-zone, people counting, fall detection |
| [IKEA MYGGSPRAY](https://www.ikea.com/fi/fi/p/myggspray-langaton-liiketunnistin-aelylaite-70604186/) | Matter/Thread | EUR 9.99 | **PIR motion, not presence.** Drops you when you sit still |
...
- **IKEA KAJPLATS** — 11 bulbs, Matter/Thread, cheapest Matter bulbs going. In
  Finnish stores since [January 2026](https://mobiili.fi/2025/11/20/ikea-paljasti-myyntiintulon-aikataulun-suomessa-tallaisia-ovat-uudet-matter-yhteensopivat-alykotituotteet-valaistukseen-antureihin-ja-ohjaukseen/).
...
- **[Shelly 1 / 1PM / 2PM Gen4](https://www.shelly.com/products/shelly-1pm-gen4)** — in-wall relays speaking Wi-Fi, Zigbee *and*
...
- **SAVY Zigbee** switches from [SuperLED.fi](https://www.superled.fi/category/335/zigbee-himmentimet-ja-kytkimet), including a kojerasia model.

### Other IKEA 2026 Matter/Thread sensors

MYGGBETT (door/window), ALPSTUGA (air quality), KLIPPBOK (water leak),
GRILLPLATS (smart plug, later release). All [arrived in Finland January 2026](https://mobiili.fi/2025/11/20/ikea-paljasti-myyntiintulon-aikataulun-suomessa-tallaisia-ovat-uudet-matter-yhteensopivat-alykotituotteet-valaistukseen-antureihin-ja-ohjaukseen/).
...
4. **Retailers.** Verkkokauppa.com, Proshop.fi, [Nurkan takaa](https://verkkokauppa.nurkantakaa.fi/tuote/smlight-slzb-mr4/) (also a Hyrylä
   ...
   [Domadoo (FR)](https://www.domadoo.fr/en/403_smlight) ships within the EU and was consistently cheaper on SMLIGHT.
